...
final class NonEmptyMap<K, T> {
  ...
  static Optional<NonEmptyMap<K, T>> fromIMap<K, T>(IMap<K, T> map) {
    ...
    iterator.moveNext();
    final firstEntry = iterator.current;

    final restMap = map.remove(firstEntry.key);
  ...
  Optional<NonEmptyMap<K, T>> removeWhereKeyed(bool Function(K key, T value) predicate) {
